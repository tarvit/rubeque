Feature: Submit solutions
  ...
  Scenario: A user tries to submit malicious code
    ...
    When I go to the problem page for "The Truth"
    And I fill in "Kernel.exit!" for the solution code
    And I submit the solution
    Then I should see a "SystemExit" error message

  Scenario: A user submits an answer with double underscores in it
    Given I am logged in as a user named "kierkegaard"
    When I go to the problem page for "Method Acting"
    And I fill in "__method__" for the solution code
    And I submit the solution
    Then I should see a success message
    And there should be a new solution in the database

  Scenario: A user submits a regex solution
    Given I am logged in as a user named "kierkegaard"
    When I go to the problem page for "Quelle Heure Est-Il?"
    And I fill in a regex date solution
    And I submit the solution
    Then I should see a success message
    And there should be a new solution in the database
